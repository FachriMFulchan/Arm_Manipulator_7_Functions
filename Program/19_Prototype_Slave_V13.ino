...
// Variable untuk menampung data degree
int T1 = 0;
int T2 = 0;
int T3 = 0;
int T4 = 0;
...
void receiveEvent() {
  ...
  if (parsing) {
    ...
    if (dataInt[1] == 1) {
      ...
      currentStateJ6 = thetaConversions(T6, minAngleJ6, maxAngleJ6, batasJ6Min, batasJ6Plus);
...
int thetaRollConversions (int angle, int minStep, int maxStep) {
  ...
  if (angle > 0) {
    compare = round((angle / 360) * maxStep);
  }
  else if (angle < 0) {
    compare = round((angle / -360) * minStep);
  }
  return compare;
}
